...
# 抽取特征函数
def ReadSentence(raw_file, feature_file):
        ...
        while len(passage) > 0:
            ...
            while "passage" not in line and len(line) > 0:
                ...
                if ContainEntity(line):
                    l = len(line)
                    all_entities = ExtractAllEntities(line)
                    line = line.split()
                    # 处理每一对实体
                    for key, value in all_entities.items():
                        ...
                        if chemical_pos > disease_pos:
                            order = 1
                        if order == 0:
                            others = ContainOthers(line[chemical_pos + 1:disease_pos])
                        else:
                            others = ContainOthers(line[disease_pos + 1:chemical_pos])
...
def ExtractAllEntities(sentence):
    # 存储化学物质实体，疾病实体，结果集合
    chemical_entities = []
    disease_entities = []
    result = {}
...
if __name__ == "__main__":
    ...
    ReadCID("replace/train.txt")
    print(ContainOthers(sentence.split()))
